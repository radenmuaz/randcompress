...
- `../enfrac_baseline/` (sibling package) — ported from `overfitter/`: the plain, no-adapter
...
- `configs/` — sample dataset configs for **this** (HiRA/PEFT) package: fatihah/juz1/quran_uthmani,
  ...
  `compress` dict. `../enfrac_baseline/configs/` holds the no-HiRA counterparts (same
  architecture dims and dataset/step settings, minus `use_hira`/`hira_r` since
  `enfrac_baseline.model.ModelConfig` has no such fields) -- the two directories are separate
...
silently ignored. `enfrac_baseline.train`/`.compress` take the same `--config` flag against
their own (HiRA-free) `ModelConfig` -- point them at `enfrac_baseline/configs/*.py` instead.
...
visible -- no flag needed, and `enfrac_baseline.train` inherits this for free since it calls the
...
`enfrac/model.py` and `enfrac_baseline/model.py` instead store exactly one `Trunk` and call it
