...
PRINT_EVERY       = 20    # loss-only print interval
...
def main():
    ...
    for seg_idx in range(n_segs):
        ...
        for it in range(1, MAX_ITER_PER_PHASE + 1):
            ...
            do_check = (it % CHECK_EVERY == 0) or (it == 1) or (it == MAX_ITER_PER_PHASE)
            do_print = (it % PRINT_EVERY == 0) or (it == 1)
            ...
            if do_check:
                ...
                elapsed = time.perf_counter() - t0
                print(f"  [solo it={it:5d}] loss={float(loss):.5f}  {elapsed:.1f}s")
                sys.stdout.flush()
        ...
        if seg_idx == 0:
            # No combined phase for first segment
            continue
        ...
        for it in range(1, MAX_ITER_PER_PHASE + 1):
            ...
            if do_check:
                ...
                elapsed = time.perf_counter() - t0
                print(f"  [comb it={it:5d}] loss={float(loss):.5f}  {elapsed:.1f}s")
                sys.stdout.flush()
