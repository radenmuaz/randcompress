"""
randcompress v9 — Isotropic Pre-activation Whitening for SinkGD optimality

Changes from v4.4.2.1:

SinkGD theory: for a linear layer y=Wx, one gradient step with lr=1 reaches the
quadratic optimum iff the input covariance E[xxᵀ]=I (isotropic/white inputs).
HiRA Hessian w.r.t. B: H_B = A(E[xxᵀ]⊙W₀W₀ᵀ)Aᵀ. Since A has orthonormal rows
and W₀ is orthonormal, H_B=σ²I iff E[x_i²]=σ² for all i (isotropic marginals).

The violation: xa = silu(x_m) has E[silu(z)]≈0.17≠0 and E[silu(z)²]≈0.26≠1,
introducing a rank-1 mean-shift in E[xa xaᵀ] that inflates one singular value
of H_B and breaks single-step convergence.

The fix (this version):
  1. Activation LayerNorm: wrap every silu output with layer_norm before it
     enters any weight projection. This enforces E[xa]=0 and E[xa_i²]=1,
     making H_B=I_r and SinkGD at lr=1 an exact Newton step on the local
     quadratic.
     - mLSTMParams: new frozen act_ln_w [inner], act_ln_b [inner]
     - sLSTMParams: new frozen act_ln_w [d_model], act_ln_b [d_model]
     - Applied in all six layer functions (parallel/step/with_init for each cell)
  2. HiRA B column-centering: after each SinkGD update, subtract column means
     of every B matrix. Maintains E[Bv]≈0 for zero-mean inputs v.
  3. Default learning_rate=1e-2: Sinkhorn fixes ‖G̃‖_F=√(mn) regardless of raw
     gradient scale, so effective step size = √(mn)·lr. For output_proj 64×256,
     √(mn)=128, so lr=1/128≈0.008. "lr=1 exact" applies to raw gradient, not
     post-Sinkhorn. lr=1e-2 keeps all matrices in a stable regime.
...
class mLSTMParams(NamedTuple):
    ...
    ln_b:     jnp.ndarray
    W_down:   jnp.ndarray
    act_ln_w: jnp.ndarray   # frozen LN on silu(x_m): enforces E[xa]=0, E[xa²]=1
    act_ln_b: jnp.ndarray
...
class sLSTMParams(NamedTuple):
    ...
    gn_w:     jnp.ndarray
    gn_b:     jnp.ndarray
    act_ln_w: jnp.ndarray   # frozen LN on silu(x): enforces isotropic input to W_i/f
    act_ln_b: jnp.ndarray
...
def apply_mlstm_hira(p: mLSTMParams, l: mLSTMHiRA) -> mLSTMParams:
    ...
    return mLSTMParams(
        ...
        act_ln_w=p.act_ln_w, act_ln_b=p.act_ln_b,
    )
...
def mlstm_layer_parallel(p: mLSTMParams, x: jnp.ndarray, num_heads: int):
    ...
    xa      = layer_norm(jax.nn.silu(x_m), p.act_ln_w, p.act_ln_b)
...
def slstm_layer(p: sLSTMParams, x: jnp.ndarray, num_heads: int):
    B = x.shape[0]
    H = p.W_i.shape[0]
    xa = layer_norm(jax.nn.silu(x), p.act_ln_w, p.act_ln_b)
...
def slstm_layer_step(p: sLSTMParams, x_t: jnp.ndarray, state, num_heads: int):
    sy, sc, sn, sm = state
    xa = layer_norm(jax.nn.silu(x_t), p.act_ln_w, p.act_ln_b)
...
def init_mlstm_params(key, d_model, num_heads, seq_len) -> mLSTMParams:
    ...
    return mLSTMParams(
        ...
        W_down=W_down,
        act_ln_w=jnp.ones((inner,), DTYPE),
        act_ln_b=jnp.zeros((inner,), DTYPE),
    )
...
def init_slstm_params(key, d_model, seq_len) -> sLSTMParams:
    ...
    return sLSTMParams(W_i=W_i, W_f=W_f, W_z=W_z, W_o=W_o,
                       ...
                       act_ln_w=jnp.ones((H,), DTYPE),
                       act_ln_b=jnp.zeros((H,), DTYPE))
