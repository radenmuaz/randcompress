"""
randcompress with HiRA — Persistent TBPTT (single file)

Base xLSTM: random, frozen. Trainable: RandCompressParams (output_proj + HiRA).
Training strategy: Persistent TBPTT — the dataset is split into non-overlapping
chunks of fixed length. Within each epoch the hidden state (C, n, m, conv_buf for
mLSTM; y, c, n, m, conv_buf for sLSTM) is carried across consecutive chunks so the
model sees long-range context. A stop_gradient is applied at every chunk boundary so
gradients do not flow back across chunks (truncated BPTT). At the start of each epoch
the state is reset to zeros.
...
def apply_mlstm_hira(p: mLSTMParams, l: mLSTMHiRA) -> mLSTMParams:
    ...
    dm = NH * DH
    def qkv(w, a):
        return apply_hira(w.reshape(dm, inner_dim), a).reshape(NH, DH, inner_dim)
    return mLSTMParams(
...
def main():
    ...
    config = Config(
        ...
        learning_rate=1e-3, weight_decay=0, grad_clip_norm=10.0,
        grad_accum_k=1,
        # grad_accum_k=4,
    )
    ...
    num_epochs  = 20
    total_steps = num_epochs * num_chunks   # LR schedule in chunk steps
    global_step = 0                         # chunk steps (not update steps)
    t_total     = 0.0
    ...
    for epoch in range(num_epochs):
        states     = init_step_states(config, batch_size=1)
        epoch_loss = 0.0
        recent     = []   # block-average losses for rolling avg_bpc
        ...
        for block_start in range(0, num_chunks, K):
            ...
            k_actual  = block_end - block_start   # last block may be smaller

            accum_grads = None
            block_loss  = 0.0
            t0 = time.perf_counter()

            # ---- accumulate gradients over k_actual chunks (params fixed) ----
            for ci in range(block_start, block_end):
                ...
                loss_f     = float(loss)   # sync here keeps timing honest
                block_loss += loss_f
                global_step += 1
            ...
            # ---- average gradients, single param update ----
            if k_actual > 1:
        ...
        avg_ms   = t_total / max(global_step, 1)   # ms per chunk step
    ...
    # teacher-forced BPC on train (chunked, stateful)
