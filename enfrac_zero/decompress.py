...
across one file CHUNK's timesteps at a time (see model.py's ByteFractalGen.__call__ docstring /
train.py's make_file_chunks) -- decoding is autoregressive at level 0 by necessity within each
chunk (future bytes genuinely unknown), one call to generate() per BATCH of chunks (chunk_batch_size
independent chunks decoded in LOCKSTEP -- see generate()'s docstring: still fully sequential in
time within each chunk, real autoregression, but chunk_batch_size chunks advance together, each
keeping its OWN independent RCDecoder state -- "stateful between chunks" is handled by simply
holding chunk_batch_size separate decoder objects, one per chunk in the batch), using each
chunk's OWN independent RC stream (see compress.py's encode() docstring for why streams are kept
separate rather than concatenated: chunks never attend across each other, so nothing about
compress ever needed them to share one stream). `device` must match compress.py's choice
(TPU/GPU/CPU matmuls aren't bit-identical to each other, see load_bundle() below); `dtype` must
ALSO match (see compress.py's module docstring) -- float32 vs float64 arithmetic produces
different logits, which would desync the range coder just as fatally as a device/batch_size
mismatch. `file_chunk_bytes`/`n_file_chunks`/`stream_lengths`/`chunk_batch_size` are further
invariants of the same kind -- decoding chunk c with the wrong stream, chunk_n_timesteps, or
batch_size desyncs that chunk's range coder exactly like a dtype/device mismatch would (batch_size
specifically because collect_logits_batched()'s logits differ, by ~1e-6, from a DIFFERENT batch
size's -- see its docstring: compress and decompress must agree with EACH OTHER at the exact same
batch_size, not with some reference). All of these are read from meta.json automatically, no CLI
override.
"""
...
def decode(bundle_dir: str, output_path: str, verify_path: str | None = None) -> bytes:
    ...
    for start in tqdm(range(0, n_file_chunks, chunk_batch_size), desc=desc, unit="batch"):
        ...
        def symbol_fn(logits_batch, decoders=decoders) -> list:
            ...
            # rc_decode_step), not reimplemented in Python -- see enfrac/codec.py's RCDecoder
            # docstring. IMPORTANT: preserve logits_batch's own dtype here -- downcasting would
            # compute a DIFFERENT CDF than compress.py did, desyncing the range coder (see
            # compress.py's docstring). Each of the B chunks in this batch uses its OWN decoder --
            # this IS "handling the stateful case between batch members": B independent decoder
