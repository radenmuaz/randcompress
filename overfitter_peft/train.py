...
def train(model: ByteFractalGen, chunks: np.ndarray, device: torch.device, steps: int,
    ...
    param_bytes = sum(pp.numel() * pp.element_size() for pp in trainable)
...
def main() -> None:
    ...
    p.add_argument("--use_hira", type=lambda x: x.lower() != "false", default=True,
                   help="PEFT-fork default: frozen-base HiRA linears (only rank-r B trainable). "
                        "false = plain trainable nn.Linear, matching overfitter/'s baseline.")
    p.add_argument("--hira_r", type=int, default=4, help="HiRA rank (only used when use_hira=true)")
    ...
    print(f"params={n_params:,} (trainable={n_trainable:,})  use_hira={cfg.use_hira} "
          f"hira_r={cfg.hira_r}  seed={cfg.seed}  seq_lens={model.seq_lens}")
