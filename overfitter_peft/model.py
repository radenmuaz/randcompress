"""ByteFractalGen (PEFT fork): byte-level FractalAR (arxiv.org/html/2502.17437v2), same
recursive architecture as overfitter/model.py, but every Linear defaults to a HiRA-adapted
frozen-base layer instead of a plain trainable nn.Linear -- see randcompress/randcompress/
models/hira.py for the original HiRA design this is ported from (W = W0 + W0*(B@A), only B
trainable, W0/A regenerated deterministically from cfg.seed, never saved).

Fork rationale: this trades the parent overfitter/'s "small model, everything trained directly"
tradeoff for randcompress/'s "arbitrarily large frozen base, tiny trainable adapter" tradeoff --
d_model/mlp_mult/byte_embed_dim become nearly free to grow (O(d) compute, O(r) storage per
layer) instead of directly taxing the compressed bundle size. use_hira=True is the default and
main mode of this fork (set use_hira=False on FractalConfig to fall back to plain nn.Linear,
matching the parent package's baseline exactly).

IMPORTANT determinism contract: HiraLinear's frozen (W0, A) are drawn from a single
torch.Generator threaded through construction in a FIXED order (ByteFractalGen -> Trunk ->
Block -> Attn/SwiGLU, plus patch_in/cond_proj/head). Reconstructing a bundle from
(config, seed) alone requires this exact construction order to never change -- reordering
module construction between save and load silently regenerates a DIFFERENT frozen base.

--- original overfitter/model.py docstring below, still applies to the shared recursion/
...
def init_hira_A(d_out: int, d_in: int, r: int, gen: torch.Generator) -> torch.Tensor:
    """A: [r, d_in] orthonormal rows (right singular vectors of a random matrix) -- frozen."""
    raw = torch.randn(r, d_in, generator=gen)
    if r <= d_in:
        _, _, Vt = torch.linalg.svd(raw, full_matrices=False)
...
class HiraLinear(nn.Module):
    """W = W0 + W0*(B@A) (HiRA) -- W0 [d_out,d_in] and A [r,d_in] frozen (drawn from `gen` at
    ...
    def __init__(self, d_in: int, d_out: int, r: int, gen: torch.Generator):
        super().__init__()
        W0 = torch.empty(d_out, d_in)
        nn.init.orthogonal_(W0, generator=gen)
        self.register_buffer("W0", W0 / math.sqrt(d_in), persistent=False)
        self.register_buffer("A", init_hira_A(d_out, d_in, r, gen), persistent=False)
...
def make_linear(d_in: int, d_out: int, cfg: "FractalConfig", gen: torch.Generator) -> nn.Module:
    if cfg.use_hira:
        return HiraLinear(d_in, d_out, cfg.hira_r, gen)
...
class Attn(nn.Module):
    def __init__(self, d_model: int, n_heads: int, cfg: "FractalConfig", gen: torch.Generator):
        super().__init__()
        self.n_heads = n_heads
        self.head_dim = d_model // n_heads
        self.wq = make_linear(d_model, d_model, cfg, gen)
        self.wk = make_linear(d_model, d_model, cfg, gen)
        self.wv = make_linear(d_model, d_model, cfg, gen)
        self.out = make_linear(d_model, d_model, cfg, gen)
...
class SwiGLU(nn.Module):
    def __init__(self, d_model: int, mlp_mult: int, cfg: "FractalConfig", gen: torch.Generator):
        super().__init__()
        hidden = mlp_mult * d_model
        self.gate = make_linear(d_model, hidden, cfg, gen)
        self.up = make_linear(d_model, hidden, cfg, gen)
        self.down = make_linear(hidden, d_model, cfg, gen)
...
class Block(nn.Module):
    def __init__(self, d_model: int, n_heads: int, mlp_mult: int, cfg: "FractalConfig", gen: torch.Generator):
        super().__init__()
        self.ln1 = RMSNorm(d_model)
        self.attn = Attn(d_model, n_heads, cfg, gen)
        self.ln2 = RMSNorm(d_model)
        self.mlp = SwiGLU(d_model, mlp_mult, cfg, gen)
...
class Trunk(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, n_layers: int, mlp_mult: int,
                 cfg: "FractalConfig", gen: torch.Generator):
        super().__init__()
        self.d_model = d_model
        self.n_heads = n_heads
        self.head_dim = d_model // n_heads
        self.blocks = nn.ModuleList([Block(d_model, n_heads, mlp_mult, cfg, gen) for _ in range(n_layers)])
...
class FractalConfig:
    ...
    use_hira: bool = True                                  # PEFT-fork default; False = plain nn.Linear (parent's baseline)
    hira_r: int = 4                                        # HiRA rank -- only used when use_hira=True
    seed: int = 0                                           # drives frozen W0/A -- must match at load time to reconstruct
...
class ByteFractalGen(nn.Module):
    def __init__(self, cfg: FractalConfig):
        ...
        # Single generator threaded through ALL frozen-weight construction below, in a FIXED
        # order (see module docstring's determinism contract) -- this IS the "seed" that lets a
        # HiRA bundle reconstruct its frozen base from cfg.seed alone.
        gen = torch.Generator().manual_seed(cfg.seed)

        # Frozen, maximally-separated byte representation -- fixed data, never trained (see
        # module docstring for why this must stay frozen). Independent of `gen`/HiRA -- this was
        # already frozen-by-construction in the parent package, not part of the PEFT mechanism.
        ...
        if cfg.share_trunk:
            ...
            shared = Trunk(d0, h0, l0, m0, cfg, gen)
            self.trunks = nn.ModuleList([shared] * self.n_levels)
        else:
            self.trunks = nn.ModuleList([
                Trunk(cfg.d_model_list[l], cfg.n_heads_list[l], cfg.n_layers_list[l], cfg.mlp_mult_list[l], cfg, gen)
                for l in range(self.n_levels)
            ])
        ...
        # levels (child_len=1 there, just a per-token projection, not really a "patch"). This is
        # the dominant param-count site in the parent package -- HiRA-adapting it is what makes
        # byte_embed_dim/d_model nearly free to grow in this fork.
        self.patch_in = nn.ModuleList([
            make_linear(cfg.patch_len_list[l + 1] * cfg.byte_embed_dim, cfg.d_model_list[l], cfg, gen)
            for l in range(self.n_levels)
        ])
        ...
        self.cond_proj = nn.ModuleList([
            make_linear(cfg.d_model_list[0] if l == 0 else cfg.d_model_list[l - 1], cfg.d_model_list[l], cfg, gen)
            for l in range(self.n_levels)
        ])

        self.head = make_linear(cfg.d_model_list[-1], 256, cfg, gen)   # terminal level only
