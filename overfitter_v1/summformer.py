...
def make_norm(norm_type: str, d_model: int) -> nn.Module:
    ...
    if norm_type == "layernorm":
        return nn.LayerNorm(d_model)   # has bias, unlike RMSNorm -- its own extra shift term
    raise ValueError(f"unknown norm_type {norm_type!r} (expected rmsnorm | layernorm)")
...
class Attn(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, n_kv_heads: int | None = None, qk_norm: bool = True,
                 head_dim: int | None = None, use_rope: bool = True, use_bias: bool = False,
        ...
        self.wq = nn.Linear(d_model, n_heads * self.head_dim, bias=use_bias)
        self.wk = nn.Linear(d_model, self.n_kv_heads * self.head_dim, bias=use_bias)
        self.wv = nn.Linear(d_model, self.n_kv_heads * self.head_dim, bias=use_bias)
        self.out = nn.Linear(self.attn_dim, d_model, bias=use_bias)
        self.qk_norm = qk_norm
        self.use_rope = use_rope   # pos_scheme="none" (NoPE): skip rotation entirely, rely on
                                    # causal masking + the hierarchical cascade for position info
        if qk_norm:
            self.q_norm = make_norm(norm_type, self.head_dim)
            self.k_norm = make_norm(norm_type, self.head_dim)
...
class SwiGLU(nn.Module):
    def __init__(self, d_model: int, mlp_mult: int, use_bias: bool = False):
        super().__init__()
        hidden = mlp_mult * d_model
        self.gate = nn.Linear(d_model, hidden, bias=use_bias)
        self.up = nn.Linear(d_model, hidden, bias=use_bias)
        self.down = nn.Linear(hidden, d_model, bias=use_bias)
...
class MLP(nn.Module):
    """Plain 2-matmul MLP (no gate) -- cheaper than SwiGLU's 3 matmuls. A literally-linear
    (no nonlinearity) MLP would just collapse into the surrounding residual stream as a single
    effective matrix, so this keeps a nonlinearity (Swish/SiLU, matching SwiGLU's own activation)
    -- "faster" here means fewer matmuls, not literally linear."""
    def __init__(self, d_model: int, mlp_mult: int, use_bias: bool = False):
        super().__init__()
        hidden = mlp_mult * d_model
        self.up = nn.Linear(d_model, hidden, bias=use_bias)
...
class NoMLP(nn.Module):
    """Identity passthrough -- no MLP at all, pure attention-only transformer. Since Block.forward
    does `x = x + self.mlp(self.ln2(x))`, returning zeros (not x) here keeps that a true no-op
    (residual alone carries x through unchanged) rather than doubling x."""
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return torch.zeros_like(x)


def make_mlp(mlp_type: str, d_model: int, mlp_mult: int, use_bias: bool = False) -> nn.Module:
    if mlp_type == "swiglu":
        return SwiGLU(d_model, mlp_mult, use_bias)
    if mlp_type == "mlp":
        return MLP(d_model, mlp_mult, use_bias)
    if mlp_type == "none":
        return NoMLP()
    raise ValueError(f"unknown mlp_type {mlp_type!r} (expected swiglu | linear | none)")
...
class Block(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, mlp_mult: int, n_kv_heads: int | None = None, qk_norm: bool = True,
                 head_dim: int | None = None, use_rope: bool = True, mlp_type: str = "swiglu",
                 use_bias: bool = False, norm_type: str = "rmsnorm"):
        super().__init__()
        self.ln1 = make_norm(norm_type, d_model)
        self.attn = Attn(d_model, n_heads, n_kv_heads, qk_norm, head_dim, use_rope, use_bias, norm_type)
        self.ln2 = make_norm(norm_type, d_model)
        self.mlp = make_mlp(mlp_type, d_model, mlp_mult, use_bias)
...
class FuseStage(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, mlp_mult: int, n_layers: int,
                 ...
                 use_rope: bool = True, mlp_type: str = "swiglu",
                 use_bias: bool = False, norm_type: str = "rmsnorm"):
        super().__init__()
        self.ln1 = nn.ModuleList([make_norm(norm_type, d_model) for _ in range(n_layers)])
        self.attn = nn.ModuleList([Attn(d_model, n_heads, n_kv_heads, qk_norm, head_dim, use_rope, use_bias, norm_type) for _ in range(n_layers)])
        self.ln2 = nn.ModuleList([make_norm(norm_type, d_model) for _ in range(n_layers)])
        self.mlp = nn.ModuleList([make_mlp(mlp_type, d_model, mlp_mult, use_bias) for _ in range(n_layers)])
...
class Config:
    ...
    pos_scheme: str = "rope"                 # "rope" | "none" (NoPE -- causal mask + hierarchy
                                              # only, no explicit position info at all).
                                              # "absolute" deferred: a byte-position lookup table
                                              # doesn't fit a multi-million-byte file well anyway.
    mlp_type:   str = "swiglu"               # "swiglu" (default, 3 matmuls) | "mlp" (2 matmuls +
                                              # Swish/SiLU, cheaper per step) | "none" (identity
                                              # passthrough, attention-only, no MLP at all)
    use_bias:   bool = False                 # True: every Linear gets a bias term (a shift, not
                                              # just a rotation/scale) -- can converge faster for
                                              # fast-overfit, at the cost of extra params (small:
                                              # O(d_model) per layer, not O(d_model^2))
    norm_type:  str = "rmsnorm"              # "rmsnorm" (default, no bias) | "layernorm" (has its
                                              # own bias/shift term, independent of use_bias above)
...
class SummTransformer(nn.Module):
    def __init__(self, cfg: Config):
        ...
        assert cfg.pos_scheme in ("rope", "none"), \
            f"pos_scheme={cfg.pos_scheme!r} not supported (expected rope | none; absolute deferred)"
        use_rope = cfg.pos_scheme == "rope"
        ...
        n_lms = self.n_fuse + 1
        if cfg.share_lm:
            first = nn.ModuleList([Block(D, cfg.n_heads, cfg.mlp_mult, cfg.n_kv_heads, cfg.qk_norm, cfg.head_dim, use_rope, cfg.mlp_type, cfg.use_bias, cfg.norm_type) for _ in range(cfg.n_layers)])
            self.lms = nn.ModuleList([first] * n_lms)
        else:
            self.lms = nn.ModuleList(
                [nn.ModuleList([Block(D, cfg.n_heads, cfg.mlp_mult, cfg.n_kv_heads, cfg.qk_norm, cfg.head_dim, use_rope, cfg.mlp_type, cfg.use_bias, cfg.norm_type) for _ in range(cfg.n_layers)])
                 for _ in range(n_lms)])
        if cfg.share_lm:
            first_ln = make_norm(cfg.norm_type, D)
            self.ln_fs = nn.ModuleList([first_ln] * n_lms)
        else:
            self.ln_fs = nn.ModuleList([make_norm(cfg.norm_type, D) for _ in range(n_lms)])

        self.head = nn.Linear(D, V, bias=cfg.use_bias)
        if cfg.weight_tie:
        ...
        if cfg.share_fuse:
            first_fs = FuseStage(D, cfg.n_heads, cfg.mlp_mult, fuse_layers, cfg.n_kv_heads, cfg.qk_norm, cfg.head_dim, use_rope, cfg.mlp_type, cfg.use_bias, cfg.norm_type)
            self.fuse_stages = nn.ModuleList([first_fs] * self.n_fuse)
        else:
            self.fuse_stages = nn.ModuleList(
                [FuseStage(D, cfg.n_heads, cfg.mlp_mult, fuse_layers, cfg.n_kv_heads, cfg.qk_norm, cfg.head_dim, use_rope, cfg.mlp_type, cfg.use_bias, cfg.norm_type) for _ in range(self.n_fuse)])
        self.fuse_windows = resolve_fuse_window(cfg.fuse_window, self.n_fuse)

        self.extra_heads = nn.ModuleList(
            [nn.Linear(D, V, bias=cfg.use_bias) for _ in range(max(0, cfg.mtp_heads - 1))])
