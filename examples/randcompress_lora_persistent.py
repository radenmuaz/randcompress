"""
randcompress with LoRA — Persistent TBPTT (single file)
...
def main():
    log_path = "train_lora_persistent.log"
    ...
    n_real = len(tokens)
    chunk_size = 2048   # tokens per chunk (input length)
    # chunk_size = 512
    ...
    config = Config(
        ...
        learning_rate=1e-3, weight_decay=1e-6, grad_clip_norm=1.0,
    )
    ...
    compress_ratio = n_real / param_bytes   # dataset bytes / trainable model bytes
    ...
    print(f"\nPersistent TBPTT for {num_epochs} epochs... log -> {log_path}")
    print("-" * 60)
    ...
    for epoch in range(num_epochs):
        ...
        pbar = tqdm(chunks, desc=f"E{epoch+1:04d}", unit="chunk",
                    dynamic_ncols=True, leave=True)
        for chunk_np in pbar:
            ...
            params, opt_state, loss, states = train_chunk_persistent(
                base_xlstm, params, opt_state, chunk_batch, frozen_states, config, lr
            )
    ...
    # Generation check: generate first 200 bytes from seed
