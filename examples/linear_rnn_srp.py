...
def _quantize_cdf(logits_1d):
    ...
    # correct sum to exactly M: absorb rounding error into the largest entry
    deficit = M - freqs.sum()
    max_idx = jnp.argmax(freqs)
    freqs   = jnp.maximum(jnp.int32(1), freqs.at[max_idx].add(deficit))
...
def collect_probs(base_xlstm, params, config, all_inputs, all_targets):
    """Step-by-step forward using _fwd_step_jit — identical code path to the
    decompressor — so CDFs are bit-exact between encode and decode.

    Chunked forward (forward_train_chunked) produces different XLA op ordering
    than step-wise (forward_step), causing logit divergence and RC decode failure.
    Both give correct training gradients, but only step-wise gives matching CDFs.

    Returns:
      logits_np [T, V]  float32  — head-0 logits at every position (incl. padding)
    ...
    pbar = tqdm(range(T_total), desc="collect probs", unit="tok",
    ...
    for ci in range(num_chunks):
        for pos in range(chunk_size):
            ...
            logit_flat, states = _fwd_step_jit(
                ...
                config.num_heads, config.block_map, stride_map, jnp.int32(pos))
            logits = _reshape_logits(logit_flat, oh, ob)           # [1, oh, V]
            logits_list.append(np.array(logits[0, 0, :], np.float32))  # [V]
...
def eval_compression(base_xlstm, params, config, all_inputs, all_targets,
    ...
    cumfreqs = jax.vmap(_quantize_cdf)(jnp.array(vlog))   # [T_v, V+1] uint32
    ...
    rt   = 'OK' if decode_ok else 'FAIL'
    # one summary line: CE | rc | params+rc | params+resid(raw) | params+resid(rc)
    print(f"\n{hdr}  {elapsed:.1f}s  param={param_bytes}B  n_raw={n_raw_bytes}  "
          f"wrong={n_wrong}/{T_v}({acc:.1%})  {rt}")
...
def save_compressed(ckpt_dir, cstats, params, config):
    ...
    # meta
    meta = dict(
        n_raw_bytes      = cstats["n_raw_bytes"],
        T_valid          = cstats["T_valid"],
        seed_token       = cstats["seed_token"],
        rc_bytes         = cstats["rc_bytes"],
        param_bytes      = cstats["param_bytes"],   # logical (param count × dtype bytes)
        param_disk_bytes = param_disk_bytes,         # actual pkl file size (includes pickle overhead)
...
def main():
    ...
    for it in pbar:
        ...
        if it % config.check_every == 0 or it == config.max_iters:
            ...
            save_compressed(ckpt_it, cstats, params, config)
    ...
    # ── final compression eval + checkpoint ──────────────────────────────────
    print("\nFinal compression eval...")
    cstats   = eval_compression(base_xlstm, params, config, all_inputs, all_targets,
                                n_total, label="final")
    ckpt_dir = os.path.join(log_dir, "ckpt_last")
    save_compressed(ckpt_dir, cstats, params, config)
