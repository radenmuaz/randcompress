...
SEGMENT_SIZE      = 1024   # bytes per segment (= chunk_size → 1 chunk per segment)
...
def _sinkgd_normalize(g):
    ...
    return _sr_sinkhorn_2d(g32.reshape(m, n)).reshape(g32.shape)

def sinkgd_init(params):
    return None   # stateless

@jax.jit
def _sinkgd_step(grads, params, lr, weight_decay, max_norm):
    leaves = jax.tree_util.tree_leaves(grads)
    gnorm  = jnp.sqrt(sum(jnp.sum(g.astype(jnp.float32)**2) for g in leaves))
    scale  = jnp.minimum(1.0, max_norm / (gnorm + 1e-6))
    grads  = jax.tree_util.tree_map(lambda g: g * scale, grads)
    norm_g = jax.tree_util.tree_map(_sinkgd_normalize, grads)
...
def sinkgd_update(state, grads, params, lr, weight_decay=0.0, max_norm=1.0):
    return _sinkgd_step(grads, params,
                        jnp.array(lr), jnp.array(weight_decay), jnp.array(max_norm)), state
...
def main():
    ...
    config = Config(
        ...
        learning_rate=5e-3, weight_decay=0.0, grad_clip_norm=10.0,
    ...
    for seg_idx in range(n_segs):
        ...
        # SOLO phase                                                           #
        # ------------------------------------------------------------------ #
        print(f"[SOLO] seg{seg_idx+1}  budget={MAX_ITER_PER_PHASE} iters")
        ...
        if not solo_success:
            ...
            if completed:
                ...
                for pi, (pa, pfw) in enumerate(forget_accs):
                    ...
                    print(f"  {label}: acc={pa:.1%}  first_fail={fw_str2}")
        ...
        # ------------------------------------------------------------------ #
        if completed and solo_iters == MAX_ITER_PER_PHASE:
        ...
        print(f"[COMBINED] segs 1..{n_done}  budget={MAX_ITER_PER_PHASE} iters")
