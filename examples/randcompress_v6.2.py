...
def _hadamard(m):
    """Recursive construction: H_m ∈ {±1}^{2^m × 2^m}."""
    if m == 0:
        return np.array([[1.0]])
    H = _hadamard(m - 1)
    return np.block([[H, H], [H, -H]])

def _zero_init(d_out, d_in):
    """ZerO initialization (Zhao et al., 2021 arXiv:2110.12661).
    Square / dim-decreasing → (partial) identity.
    Dim-increasing → c · H_m[:d_out, :d_in],  c = 2^{-(m-1)/2}."""
    if d_out <= d_in:
        # identity or partial identity (first d_out rows of I_{d_in})
        return np.eye(d_out, d_in)
    else:
        m = math.ceil(math.log2(d_out)) if d_out > 1 else 1
        H = _hadamard(m)          # 2^m × 2^m
        c = 2.0 ** (-(m - 1) / 2)
        return (c * H[:d_out, :d_in]).astype(np.float32)

def init_hira_adapter(key, d_out, d_in, r) -> HiRAAdapter:
    # key unused — ZerO is deterministic; kept for API compatibility
    A = _bf(_zero_init(r,     d_in))   # r × d_in
    B = _bf(_zero_init(d_out, r))      # d_out × r
    ...
    for seg_idx in range(n_segs):
        ...
        for it in pbar:
            ...
            if it % config.check_every == 0 or it == config.max_iter_per_phase:
                ...
                pbar.set_description(f"acc={acc:.1%} fail_at={fw_str} bpc={bpc:.4f}")
                if acc == 1.0:
                    solo_success = True
                    break
        pbar.close()
