...
class Config(NamedTuple):
    ...
    # power_p:       int   = 2      # mLSTM attention kernel degree; 1 = v4.4.1 baseline
    seed:          int   = 0
    stride_map:    str   = "11111111"  # one digit per layer; stride-N fires every N tokens
    # block_map:     str   = "mmmmmmmm"
...
def main():
    ...
    for seg_idx, seg in enumerate(segs):
        ...
        for it in pbar:
            # lr = cosine_lr(it, 100, 0.1, 0.01,10000)
            if chunk_idx == 0:                  # epoch boundary — reset hidden state
                states = init_step_states(config, batch_size=1)

            inp = comb_inp[chunk_idx][None, :]
            tgt = comb_tgt[chunk_idx][None, :]
            ...
            # pbar.set_postfix(loss=f"{float(loss):.4f}", bpc=f"{bpc:.3f}", lr=f"{lr:.4f}")
